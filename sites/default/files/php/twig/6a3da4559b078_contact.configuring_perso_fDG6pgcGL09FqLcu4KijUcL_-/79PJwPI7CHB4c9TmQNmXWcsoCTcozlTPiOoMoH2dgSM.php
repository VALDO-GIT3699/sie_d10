...
class __TwigTemplate_2046a861e59cf91fff1232672f2a5767 extends Template
{
...
{% set config_link = render_var(help_route_link(config_link_text, \x27entity.user.admin_form\x27)) %}
{% set permission_link_text %}{% trans %}Permissions{% endtrans %}{% endset %}
{% set permission_link = render_var(help_route_link(permission_link_text, \x27user.admin_permissions\x27)) %}
{% set adding_fields_topic = render_var(help_topic_link(\x27contact.adding_fields\x27)) %}
...
<ol>
  ...
  <li>{% trans %}Verify that permissions are correct for your site\x27s roles, including the generic <em>Anonymous user</em> and <em>Authenticated user</em>. In order to use personal contact forms, users need both <em>View user information</em> (in the <em>User</em> section, which enables them to view user profiles) and <em>Use users\x27 personal contact forms</em> (in the <em>Contact</em> section, which enables them to use contact forms if they can view user profiles).{% endtrans %}</li>
...
</ol>", "@help_topics/contact.configuring_personal.html.twig", "C:\\xampp8.2.12\\htdocs\\sie_d10\\core\\modules\\contact\\help_topics\\contact.configuring_personal.html.twig");
    }
